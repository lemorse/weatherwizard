package chartview.gui.right.wire;
...
public class ObjMaker
{
  ...
  private static final String SCHEMA_LOCATION = "wireframe.xsd";
  public final static String NAMESPACE = "http://donpedro.lediouris.net/wireframe";

  static class OCSSSResolver
  ...
  {
    return generate(fName, true);
  }

  public static int generate(String fName, boolean sym)
  {
    return generate(fName, null, 0.0D, 0.0D, 0.0D, 1.0D, 1.0D, 1.0D, 0, 
                    sym);
  }
                             ...
                             int startOffset, boolean symetric)
  {
    ...
    synchronized (parser)
    {
      try
      {
        URL validatorStream = new ObjMaker().getClass().getResource(SCHEMA_LOCATION);
        ...
        WWContext.getInstance().fireLogging(source.getName() + " is valid\n");
      }
      catch (Exception ex)
      {
      ...
      try
      {
        ...
        if (destFile == null)
        {
          ...
          String outputLocation = 
            fName.substring(0, fName.lastIndexOf(File.separator) + 1);
          WWContext.getInstance().fireLogging("Name:" + outputName + "\n");
          WWContext.getInstance().fireLogging("Output: [" + outputLocation + outputName + ".obj]\n");
        ...
        OCSSSResolver resolver = new OCSSSResolver();
        ...
        if (nl != null)
        {
          ...
          for (int i = 0; i < nl.getLength(); i++)
          {
            ...
            if (node.getNodeType() == 1 && node.getNodeName() == "plot")
            {
              ...
              fw.write("v " + (x * affX + addX) + " " + (y * affY + addY) + 
                       " " + (z * affZ + addZ) + "\n");
              nbElements++;
            }
          }
        ...
        if (nl != null)
        {
          ...
          {
            Node node = nl.item(i);
            double x = Double.parseDouble(((XMLElement)node).getAttribute("x"));
        ...
        if (nl != null)
        {
          for (int i = 0; i < nl.getLength(); i++)
          {
            Node node = nl.item(i);
            double y = 
              Double.parseDouble(((Element) node).getAttribute("y"));
